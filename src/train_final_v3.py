"""
FINAL V3 - Addresses train accuracy lagging behind validation
Key changes:
1. Lower augmentation ranges (less synthetic noise)
2. Higher initial LR (0.003) to help model fit training data
3. Label smoothing (0.1) to prevent overfitting
4. Longer Phase 1 (35 epochs) to let head learn better
5. Add BatchNormalization for stability
6. Monitor both train and val accuracy for early stopping
"""
import os
import numpy as np
...
EPOCHS = 100
PHASE1_EPOCHS = 35
MODEL_PATH = "models/rice_disease_final_v3.h5"
RESULTS_DIR = "results"
...
def build_model(num_classes):
    ...
    model.compile(
        optimizer=Adam(learning_rate=0.003),  # Higher LR to escape plateau
...
def fine_tune(model, base_model):
    print("\nFine-tuning: unfreezing top 120 layers...")
...
print("="*70)
print("FINAL V3 - GENTLE AUGMENTATION + HIGHER LR")
print("="*70)

train_gen, val_gen, test_gen, class_names = load_data()
num_classes = len(class_names)

print(f"\nPhase 1: Train head only for {PHASE1_EPOCHS} epochs, LR=0.003...")
...
plt.tight_layout()
plt.savefig(f"{RESULTS_DIR}/training_curves_v3.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(f"{RESULTS_DIR}/confusion_matrix_v3.png", dpi=150)
plt.close()
