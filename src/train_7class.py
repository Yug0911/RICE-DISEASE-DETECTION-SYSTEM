"""
OPTIMIZED 7-CLASS TRAINING - Fast convergence
Changes: Higher LR, less dropout, more unfrozen layers, no class weights
...
def build_model(num_classes):
    ...
    model.compile(
        optimizer=Adam(learning_rate=0.002),  # Higher LR for faster learning
...
def fine_tune(model, base_model):
    print("\nFine-tuning: unfreezing top 100 layers...")
    base_model.trainable = True
    # Keep first 100 layers frozen (low-level features)
...
print("="*70)
print("OPTIMIZED 7-CLASS TRAINING - Fast convergence")
print("="*70)

train_gen, val_gen, test_gen, class_names = load_data()
num_classes = len(class_names)

print(f"\nPhase 1: Train head only (frozen base) for {PHASE1_EPOCHS} epochs, LR=0.002...")
...
print("\nPhase 2: Fine-tune top 100 layers (LR=1e-5)...")
